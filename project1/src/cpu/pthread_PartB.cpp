...
// Email: nazhang@link.cuhk.edu.cm
//
...
void* grayscale_filter_thread_function(void* arg) {
    ...
    for (int y = data->start_row; y < data->end_row; y++) {
        for (int x = 0; x < data->width; x++) {
            ...
            float r_sum = get_pixel_matrix_sum(data->inputBuffer, filter, r_id, data->width, data->num_channels);
            float g_sum = get_pixel_matrix_sum(data->inputBuffer, filter, g_id, data->width, data->num_channels);
            float b_sum = get_pixel_matrix_sum(data->inputBuffer, filter, b_id, data->width, data->num_channels);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, grayscale_filter_thread_function, &threadData[i]);
    }
