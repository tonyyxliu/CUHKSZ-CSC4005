...
int main(int argc, char** argv) {
    ...
    for (int y = 1; y < input_jpeg.height - 1; y++) {
        for (int x = 1; x < input_jpeg.width - 1; x++) {
            ...
            float r_sum =
                get_pixel_matrix_sum(input_jpeg.buffer, filter, r_id,
                                     input_jpeg.width, input_jpeg.num_channels);

            float g_sum =
                get_pixel_matrix_sum(input_jpeg.buffer, filter, g_id,
                                     input_jpeg.width, input_jpeg.num_channels);

            float b_sum =
                get_pixel_matrix_sum(input_jpeg.buffer, filter, b_id,
                                     input_jpeg.width, input_jpeg.num_channels);
