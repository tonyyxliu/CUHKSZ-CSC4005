...
SparseMatrix::SparseMatrix()
    : start_(1, 0), idx_(0), val_(0), n_col_(0), n_row_(0)
{
}

bool SparseMatrix::operator==(SparseMatrix &&rhs) const
{
    return ((n_col_ == rhs.n_col_) && (n_row_ == rhs.n_row_) &&
            (start_ == rhs.start_) && (idx_ == rhs.idx_) && (val_ == rhs.val_));
}
    ...
    if (idx_.size() < min_nnz)
    {
        if (rec_nnz < min_nnz)
            throw std::logic_error(
                "recommended room " + std::to_string(rec_nnz) +
        ...
        {
            throw std::length_error("can't reallocate matrix with " +
                                    std::to_string(rec_nnz) + " elements");
        }
    }
}
